...
#define PCF8574_INPUT_ADDR  0x38 //input address for first I2C port expander
#define PCF8574_OUTPUT_ADDR 0x39 //output address for second I2C port expander
...
void loop() {
  ...
  Serial.println(potValue); //display potentiometer reading (8-980)
  ...
  switch (switches) {
    case 0:
      matrix.loadFrame(binary);
      break;
    case 1:
      matrix.loadFrame(decimal);
      break;
    default: {
...
void decimalInputMode(int potValue) {
  ...
  byte decimalValue = map(potValue, 8, 980, 0, 255);
